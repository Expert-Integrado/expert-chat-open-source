...
export function caminhosAuthVercel(home, plataforma, appData) {
  // O separador e o da PLATAFORMA pedida, nao o de quem roda: path.join puro montava caminho
  // de macOS com barra invertida quando o --prova rodava no Windows (medido 09/09/2026).
  if (plataforma === "win32") {
    const w = path.win32;
    return [w.join(appData || w.join(home, "AppData", "Roaming"), "com.vercel.cli", "auth.json")];
  }
  const p = path.posix;
  if (plataforma === "darwin") return [p.join(home, "Library", "Application Support", "com.vercel.cli", "auth.json"), p.join(home, ".vercel", "auth.json")];
  return [p.join(home, ".config", "com.vercel.cli", "auth.json"), p.join(home, ".vercel", "auth.json")];
}
